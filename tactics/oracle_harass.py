...
class PlanOracleHarass(ActBase):
    ...
    async def start(self, knowledge: Knowledge):
        ...
        self.combat: GroupCombatManager = self.knowledge.combat_manager
        self.target_zone: Zone = self.knowledge.enemy_main_zone
    ...
    async def micro_oracles(self, oracles: Units):
        self.enemy_turrets = self.knowledge.known_enemy_structures.of_type(
        ...
        for oracle in oracles:  # type: Unit
            ...
            if self.enemy_turrets.exists and self.enemy_turrets.closest_distance_to(oracle.position) < 8:
                await self.start_retreat()
                continue

            # Check retreat against enemy army
            enemy_power = ExtendedPower(self.knowledge.unit_values)
            enemies = self.knowledge.known_enemy_units_mobile.closer_than(9, oracle)
            ...
            #     self.combat.addUnit(oracle, self.knowledge.own_main_zone.center_location, MoveType.PanicRetreat)
            # el
        ...
        if self.retreat:
            target_position = self.knowledge.own_main_zone.center_location
    ...
    async def start_retreat(self):
        ...
        self.retreat = True
        self.target_position = self.knowledge.own_main_zone.center_location
            ...
            if self.target_zone is not None:
                ...
                if self.target_position is not None:
                    ...
                    for oracle in oracles:
                        ...
                        self.knowledge.roles.set_tasks(UnitTask.Scouting, oracles)
