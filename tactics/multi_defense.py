...
class ZoneDefense:
    ...
    def release(self):
        ...
        for tag in tags:
            self.knowledge.roles.clear_task(tag)
    ...
    def add(self, unit: Unit, is_defender: bool):
        ...
        if is_defender:
            ...
            self.knowledge.roles.set_task(UnitTask.Fighting, unit)
...
class PlanMultiDefense(ActBase):
    ...
    async def start(self, knowledge: Knowledge):
        ...
        for i in range(0, len(self.knowledge.expansion_zones)):
            self.zone_datas.append(ZoneDefense(knowledge, self.knowledge.expansion_zones[i]))
    ...
    def init_zones(self):
        for zone_data in self.zone_datas:
            ...
            if not zone.is_ours and zone != self.knowledge.own_main_zone:
                zone_data.release()
                continue
    ...
    def select_hard_defenders(self):
        for zone_data in self.zone_datas:
            ...
            possible_defenders = self.knowledge.roles.get_defenders(defense_required, zone_data.enemy_center)
    ...
    def select_idle_defenders(self):
        ...
        for zone_data in self.zone_datas:
            ...
            all_defenders = self.knowledge.roles.all_from_task(UnitTask.Fighting)
        ...
        if maxed_zone:
            idle = self.knowledge.roles.all_from_task(UnitTask.Idle)
    ...
    def worker_defence(self, defense_required: ExtendedPower, zone_data: ZoneDefense):
        ...
        defenders: float = 0
        all_defenders: Units = self.knowledge.roles.all_from_task(UnitTask.Defending)
